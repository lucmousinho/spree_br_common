jQuery(function($) {
  ...
  Spree.updateCity = function (region) {
    ...
    if (stateId !== '') {
      if (!Spree.cities[stateId] || !Spree.cities[stateId].cities) {
        return Spree.ajax(
          Spree.routes.cities_search,
          {
            data: {state_id: stateId},
            success: function (data) {
              Spree.cities[stateId] = data;
              Spree.fillCities(Spree.cities[stateId].cities, region);
            }
          }
        );
      } else {
        return Spree.fillCities(Spree.cities[stateId].cities, region);
      }
    }
  };
